...
@section('content')
  ...
  <a href="{{ URL::route('seller-all-products') }}" class="btn btn-primary">Back</a>

  <a href="#" class="dropdown-toggle btn btn-danger" onclick="event.preventDefault(); document.getElementById('dlt-prod-frm').submit();">
      Delete
  </a>

  <form id="dlt-prod-frm" action="{{ URL::route('seller-destroy-product', $product) }}" method="POST" style="display: none;">
      {{ csrf_field() }}
      {{ method_field('DELETE') }}
  </form>
  ...
  <a href="{{ URL::route('seller-edit-product-form', $product) }}" class="btn btn-primary">Edit</a>
